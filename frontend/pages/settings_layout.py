...
# ==========================================
# 1. KIỂM TRA BATCH DỮ LIỆU & CHỌN FILE
# ==========================================
...
# Load dữ liệu của file ĐANG ĐƯỢC CHỌN
...
with st.container(border=True):
    ...
    with col_info:
        ...
        classification = ddc if ddc else (nlm_code if nlm_code else "N/A")
    ...
    with col_score:
        st.metric(label="Độ chính xác AI", value="95%") 

st.divider()

# ==========================================
# 3. LAYOUT SONG SONG (ẢNH BỘ NHỚ RAM | BẢNG MARC21)
# ==========================================
...
with main_col_right:
    ...
    for idx, field_obj in enumerate(fields):
        ...
        # TRƯỜNG HỢP 2: Data Fields (040, 090, 245...)
            ...
            raw_subfields = field_obj.get("subfields", [])
            
            if isinstance(raw_subfields, dict):
                raw_subfields = [raw_subfields]
            ...
            for sub_idx, subfield in enumerate(raw_subfields):
                ...
                if isinstance(subfield, dict):
                    if "code" in subfield:
                        sub_code = str(subfield.get("code", ""))
                        sub_value = str(subfield.get("value", ""))
                    else:
                        sub_code = str(list(subfield.keys())[0]) if subfield.keys() else ""
                        sub_value = str(list(subfield.values())[0]) if subfield.values() else ""
                ...
                with col2:
                    ...
                    st.text_input("i1", value=ind1 if sub_idx == 0 else "", key=f"{selected_file}_i1_{tag}_{idx}_{sub_idx}", label_visibility="collapsed")
                with col3:
                    st.text_input("i2", value=ind2 if sub_idx == 0 else "", key=f"{selected_file}_i2_{tag}_{idx}_{sub_idx}", label_visibility="collapsed")
                with col4:
...
with col_btn:
    if st.button("💾 Lưu bản ghi dữ liệu", type="primary", use_container_width=True):
        ...
        for idx, field_obj in enumerate(fields):
            tag = field_obj.get("tag", "")
            if "data" in field_obj:
                # Đọc dữ liệu ra bằng key mới
                user_value = st.session_state.get(f"{selected_file}_val_{tag}_{idx}", field_obj["data"])
                ...
                # Đọc Indicator từ dòng đầu tiên của cụm subfield bằng key mới
                user_ind1 = st.session_state.get(f"{selected_file}_i1_{tag}_{idx}_0", field_obj.get("ind1", " "))
                user_ind2 = st.session_state.get(f"{selected_file}_i2_{tag}_{idx}_0", field_obj.get("ind2", " "))
                
                updated_subfields = []
                for sub_idx, subfield in enumerate(raw_subfields):
                    user_sub_code = st.session_state.get(f"{selected_file}_sub_{tag}_{idx}_{sub_idx}")
                    user_sub_val = st.session_state.get(f"{selected_file}_val_{tag}_{idx}_{sub_idx}")
                    if user_sub_code:
                        updated_subfields.append({"code": user_sub_code, "value": user_sub_val})
                
                updated_fields.append({
                    "tag": tag, "ind1": user_ind1, "ind2": user_ind2, "subfields": updated_subfields
                })
        
        # Lưu đè dữ liệu mới vào đúng file đang chọn trong kho tổng
