...
class DepartmentCorrector:
            ...
            if cap2_direct:
                ...
                if pair:
                    return {
                        ...
                        "reason": f"Khớp tên Cấp 2 '{matched_cap2}' đạt {score:.0f} điểm (ngưỡng ≥85)",
                    }
        ...
        if cap2_match:
            ...
            if pair:
                return {
                    ...
                    "match_method": "fuzzy_cap2",
                    "needs_review": score < 90,
                    "reason": f"Tìm thấy Cấp 2 '{matched_cap2}' trong chuỗi ghép (partial đạt {score:.0f} ≥75)",
                }
    ...
    def _build_cap1_result(self, matched_cap1: str, score: float, original: str):
            ...
            for cap2 in cap2_options:
                if normalize_vietnamese(cap2) in norm:
                    return {
                        ...
                        "needs_review": score < 90,
                        "reason": f"Khớp Cấp 1 '{matched_cap1}' ({score:.0f}) và Cấp 2 '{cap2}' có trong chuỗi",
                    }
        ...
        if len(cap2_options) == 1:
            return {
                ...
                "needs_review": score < 90,
                "reason": f"Khớp Cấp 1 '{matched_cap1}' ({score:.0f}); Cấp 2 duy nhất '{cap2_options[0]}'",
            }
        return {
            ...
            "needs_review": True,
            "reason": (
                f"Chỉ khớp Cấp 1 '{matched_cap1}' ({score:.0f}); có {len(cap2_options)} Cấp 2 con, không tự chọn — cần duyệt"
                if len(cap2_options) > 1
                else f"Chỉ khớp Cấp 1 '{matched_cap1}' ({score:.0f}); chưa xác định Cấp 2 — cần duyệt"
            ),
        }

    def _no_match(self, clean_name: str) -> dict:
        cap1, cap2 = self._nearest_candidates(clean_name)
        near_parts = []
        if cap1:
            near_parts.append(f"cap1 gần nhất '{cap1[0]}' = {cap1[1]}")
        if cap2:
            near_parts.append(f"cap2 gần nhất '{cap2[0]}' = {cap2[1]}")
        reason = "Không có đơn vị nào trong Phụ lục 6 đạt ngưỡng 85"
        if near_parts:
            reason += " — " + ", ".join(near_parts)
        else:
            reason += " — chuỗi quá lệch so với toàn bộ danh mục"
        return {
    ...
    def _nearest_candidates(self, clean_name: str):
        """Tìm đơn vị gần nhất ở Cấp 1 và Cấp 2 (không chặn ngưỡng) để ghi lý do no_match."""
        norm_query = normalize_vietnamese(clean_name)
        cap1 = None
        cap2 = None
        if norm_query and self.cap1_names and self.cap2_names:
            r1 = process.extract(
                norm_query, self.matcher_cap1._normalized_choices,
                scorer=fuzz.token_set_ratio, limit=1,
            )
            if r1:
                cap1 = (self.cap1_names[r1[0][2]], round(r1[0][1], 1))
            r2 = process.extract(
                norm_query, self._cap2_norm,
                scorer=fuzz.token_set_ratio, limit=1,
            )
            if r2:
                cap2 = (self.cap2_names[r2[0][2]], round(r2[0][1], 1))
        return cap1, cap2